    ...
    if db_path.exists():
        with sqlite3.connect(str(db_path)) as connection:
            ...
            for table in df['tbl_name'].values.tolist():
                ...
                create_db_schema.append(pd.io.sql.get_schema(table_df, table))

    db_create_schema = Path('sts2_run_db_schema.sql')
    db_create_schema.write_text('\n'.join(create_db_schema))
...
win_loss_query = "SELECT p.character, r.win FROM runs r JOIN players p on r.run_id = p.run_id WHERE r.was_abandoned = 0;"
